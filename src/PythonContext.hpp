...
struct PythonContextLock {
  PythonContextLock(PythonThreadContext &ctx) {
    // This is needed as a hack to prevent multiple threads from starving each other on the global
    // Python mutex. Seems to work well at least on the Linux scheduler.
